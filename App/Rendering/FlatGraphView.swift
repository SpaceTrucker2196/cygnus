...
// layout. Permanent view mode, not a placeholder — the RealityKit
// renderer joins it behind the same scene/layout inputs (S5).
...
struct FlatGraphView: View {
    ...
    private func canvas(in size: CGSize) -> some View {
        Canvas { context, size in
            ...
            let showLabels = scene.nodes.count <= 250 || currentScale(in: size) > 1.5
            for node in scene.nodes {
                ...
                context.fill(Path(ellipseIn: rect),
                             with: .color(KindStyle.color(for: node.kind)))
                if isSelected {
